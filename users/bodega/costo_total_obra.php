			...
			while(($salidas[] = mysql_fetch_assoc($result))||array_pop($salidas));
				$contador = 0;		
				 foreach( $salidas as $salida){
				 	...
				 	$materiales_temp[$contador]['costo'] = $salida['total'];
				 	$contador++;
				 }
				 $result_ent = $objEnt->list_entrada_devolucion_obra($obra['codigo']);
				 
				 if(mysql_num_rows($result_ent)!= 0){
					
					var_dump($result_ent);
					
					while(($entradas[] = mysql_fetch_assoc($result_ent))||array_pop($entradas));
						$contador_devoluciones = 0;		

						 foreach( $entradas as $entrada){
						 
						 	$devoluciones_temp[$contador_devoluciones]['codigo'] = $entrada['codmat'];
						 	$devoluciones_temp[$contador_devoluciones]['costo'] = $entrada['total'];
				...
				foreach($cod_materiales as $cod_material){
					...
					$materiales[$cod_material]['entradas'] = 0;
					$materiales[$cod_material]['salidas'] = 0;
					...
					foreach($materiales_temp as $material_temp){
					  		...
					  		if($cod_material == $material_temp['codigo']){
								...
								$materiales[$cod_material]['salidas'] += intval($material_temp['costo']);	
					...
					foreach($devoluciones_temp as $devolucion_temp){
							
							
							if($cod_material == $devolucion_temp['codigo']){
							
								$materiales[$cod_material]['entradas'] += intval($devolucion_temp['costo']);
					...
					$materiales[$cod_material]['total'] = intval($materiales[$cod_material]['salidas']) - intval($materiales[$cod_material]['entradas']);
       ...
       <div class="table-responsive"> 
        <table align="center" class="table table-striped" border="1">
            ...
            <thead>
                  ...
                  <th colspan="11" align="center">Costo total de obra: 
                  	<?php 
                  		if(isset($_POST['obra'])){ 
                  			echo $_POST['obra']; 
                  		} 
                  ...
                  <form action="costo_total_obra.php" method="post">
                  <tr>
            ...
            <tbody>
				...
				?>
                    	...
                    	<tr>
                            ...
                            <td>$ <?php echo $material['salidas'];?></td>
			    <td>-$ <?php echo $material['entradas'];?></td>
				...
				$total_obra +=	$material['total'];	
				} //fin foreach
				?>
			<tr>
				<td colspan="4"></td>
        		        <td>Total Obra</td>
                  		<td>$ <?php echo $total_obra;?></td>
                  	</tr>
				<?php
